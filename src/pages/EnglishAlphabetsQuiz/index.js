...
class Numbers extends Component {
  ...
  increment = () => {
    const i = this.randomInt(0, 25 - this.state.count - 1);
  ...
  getOptions = () => {
    ...
    for (let index = 0; index < 4; index++) {
      let i = this.randomInt(0, 25 - opts.length);
      opts.push(optList[i]);
    ...
    return (
      <div className="App">
        <header className="eng-alpha-header">
          <div className="welcome-text">Alphabets (अल्फाबेट्स - इंग्रजी अक्षरे) Quiz</div>
        </header>
        {this.state.rerenderChild
          && <Child
            wait={5000}
            color={this.randomInt(0, 25)}
